"""
RetinexNet decomposition + relight networks, vendored from a PyTorch port of
the original "Deep Retinex Decomposition for Low-Light Enhancement" paper.

Source:  https://github.com/aasharma90/RetinexNet_PyTorch
Path:    model.py (branch: master)

Only the DecomNet and RelightNet classes are reproduced here, since this
project performs inference only. The trainer / loss / dataset code from the
upstream file is omitted. Variable names and layer definitions are unchanged.
"""
import torch
...
class RelightNet(nn.Module):
    def __init__(self, channel=64, kernel_size=3):
        ...
        self.net2_conv0_1 = nn.Conv2d(
            4, channel, kernel_size, padding=1, padding_mode="replicate"
        )
